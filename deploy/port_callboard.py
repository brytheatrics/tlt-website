...
SHIM = r'''
  ...
  const CB_WRITE_ROUTES = {
    ...
    moveContactSheetToShow: {
      // MOVE the doc into {season}/{show}/General/. Personal Drive can't
      // multi-parent, so the doc leaves the Contact Sheets folder. Regenerate
      // sees the new parent (via URL cache) and stays there.
    ...
    moveTechScheduleToShow: {
      // Same as contact sheet's move, for the tech schedule.
  ...
  function cbShowContactSheetModal(showName, url) {
    ...
    overlay.innerHTML =
      '<div style="background:#fff; border-radius:10px; padding:24px; max-width:460px; width:calc(100% - 40px); box-shadow:0 4px 24px rgba(0,0,0,0.15);">' +
        ...
        '<p style="margin:0 0 20px; color:#666; font-size:13px;">Move to show Drive once the cast + team are locked in — the doc leaves the Contact Sheets folder and lives in the show\\'s <em>General</em> folder from then on. Future regenerates stay there.</p>' +
        '<div style="display:flex; gap:8px; justify-content:flex-end; flex-wrap:wrap;">' +
          ...
          '<button id="cb-cs-move"   class="btn btn-secondary" style="font-size:13px; padding:6px 14px;">Move to show Drive</button>' +
    ...
    document.getElementById('cb-cs-move').onclick = function () {
      var btn = document.getElementById('cb-cs-move');
      btn.disabled = true; btn.textContent = 'Moving…';
      google.script.run
        .withSuccessHandler(function (r) {
          close();
          cbShowDocReady('Moved to ' + (r && r.folder ? r.folder : (showName + ' / General')), showName, url);
        })
        .withFailureHandler(function (err) {
          btn.disabled = false; btn.textContent = 'Move to show Drive';
          showAlert('Could not move to show Drive: ' + (err && err.message ? err.message : 'unknown error'));
        })
        .moveContactSheetToShow(showName);
    };
  }
  ...
  function cbShowScheduleModal(showName, url) {
    ...
    overlay.innerHTML =
      '<div style="background:#fff; border-radius:10px; padding:24px; max-width:460px; width:calc(100% - 40px); box-shadow:0 4px 24px rgba(0,0,0,0.15);">' +
        ...
        '<p style="margin:0 0 20px; color:#666; font-size:13px;">Move to show Drive once the tech dates are finalized — the doc leaves Tech Packets and lives in the show\\'s <em>General</em> folder from then on. Future regenerates stay there.</p>' +
        '<div style="display:flex; gap:8px; justify-content:flex-end; flex-wrap:wrap;">' +
          ...
          '<button id="cb-sched-move"   class="btn btn-secondary" style="font-size:13px; padding:6px 14px;">Move to show Drive</button>' +
    ...
    document.getElementById('cb-sched-move').onclick = function () {
      var btn = document.getElementById('cb-sched-move');
      btn.disabled = true; btn.textContent = 'Moving…';
      google.script.run
        .withSuccessHandler(function (r) {
          close();
        ...
        .moveTechScheduleToShow(showName);
    };
  }
