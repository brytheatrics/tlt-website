  ...
  (function () {
    ...
    function ensureResendUI() {
      ...
      container.querySelectorAll('tbody tr').forEach(function (tr) {
        ...
        // Dedup: skip if the cell already has a working Send / Resend /
        // Send Combined button (from either _renderPreviewSendButtons or
        // cbBuildActionButtons). Only inject when the row is missing one.
        var existingSends = Array.from(actionsCell.querySelectorAll('button')).filter(function (b) {
          var t = (b.textContent || '').trim();
          return t === 'Send' || t === 'Resend' || t === 'Send Combined';
        });
        if (existingSends.length > 0) return;
